...
class FASDataset(Dataset):
    ...
    def __init__(self, root_dir, depth_map_size, transform, smoothing):
        ...
        self.data = [f for f in os.listdir(root_dir) if os.path.isfile(os.path.join(root_dir, f))]
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.altsep.join([self.root_dir, img_name])
        ...
        if label == 1:
            ...
            depth_map = np.ones((self.depth_map_size[0], self.depth_map_size[1]), dtype=np.float32) * (
                    1.0 - self.label_weight)

        if self.transform:
            img = self.transform(img)

        # plt.imshow(ToPILImage(img), interpolation='nearest')
        # plt.show()
